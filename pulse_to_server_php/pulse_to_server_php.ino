...
char sensorId[] = "QandM"; //This should not contain a space ' ' char
...
void setup() {
  ...
  Serial.println("Starting");
	...
	Serial.println(site);
        ...
        /*Send Basic Info TO AWS */
        wifly.println("GET /pulse/receive.php?pulse=wifly_bootup HTTP/1.0");        
        wifly.println();
...
void loop() {
  ...
  if (QS == true){                       // Quantified Self flag is true when arduino finds a heartbeat
    ...
    if(currentMillis +time > lastSend){
      sendDataToAWS(BPM);
...
void sendDataToAWS(int data){
  ...
  if (wifly.open(site, 80)) {
    ...
    sprintf(buffer, "GET /pulse/receive.php?pulse=%d HTTP/1.0", data);
    wifly.println(buffer);        
    wifly.println();
  }else {
    Serial.println("Data Connection Error");
  }
}
/*
